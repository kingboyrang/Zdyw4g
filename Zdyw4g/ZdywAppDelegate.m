...
{
    NSLog(@"url = %@", url);
    if([[url scheme] isEqualToString:kZdywBrandID])    //支付宝
    {
...
- (void)initAppSetting{
    ...
    [usDeaults setObject:kZdywBrandID forKey:kZdywDataKeyBrandID];
    [usDeaults setObject:kZdywPhoneType forKey:kZdywDataKeyPlateVersion];
    [usDeaults setObject:kZdywPublicKey forKey:kZdywDataKeyServerKey];
    [usDeaults setObject:kZdywHttpServer forKey:kZdywDataKeyServerAddress];
        ...
        if ([expirationDateStr length]) {
            ...
            if ([expirationDate timeIntervalSinceDate:[NSDate date]]<15*24*60*60)
            {
                if (![[ZdywUtils getLocalStringDataValue:kExpirationDateTip] isEqualToString:[dateFormatter stringFromDate:[NSDate date]]])
                {
                    ...
                    NSString *msgStr = [NSString stringWithFormat:@"你当前使用的企业版说说电话%@就要过期了，赶紧更新最新的版本，谢谢支持",dateStr];
    ...
    switch ([[dic objectForKey:@"result"]intValue])
    {
        case 0:
        {
            ...
            if([tempDic count]>0)
            {
                ...
                {
                    [ZdywUtils setLocalDataString:customerPhone key:kCustomerPhone];
                }
                //保存客服服务时间
                NSString *serviceTime = [tempDic1 objectForKey:@"service_time"];
                if ([serviceTime length] > 0) {
                    [ZdywUtils setLocalDataString:serviceTime key:kServiceTime];
                }
                //保存充值界面的充值方式
